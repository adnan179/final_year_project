...
const ReviewerDetails = () => {
  const { adminEmail, reviewerEmployeeId } = useParams();
  const [reviewer, setReviewer] = useState();
  const [loading, setLoading] = useState();

  useEffect(() => {
    axios
      .get(`http://localhost:4000/reviewers/${reviewerEmployeeId}`)
      .then((response) => {
      ...
      <div className="flex flex-col w-[80%] h-screen py-5 px-5 mt-[100px] ml-[70px] text-white gap-6">
        ...
        <div className="flex flex-col gap-10 pl-16">
          ...
          <h2 className="font-medium font-poppins text-xl">
            Employee ID: {reviewer.employeeId}
          </h2>
          <h2 className="font-medium font-poppins text-xl">
          ...
          <h2 className="font-medium font-poppins text-xl">
            Phone Number: {reviewer.phoneNumber}
          </h2>
          <h2 className="font-medium font-poppins text-xl">
            Office Room Number: {reviewer.officeRoomNumber}
          </h2>
        </div>
      </div>
    </div>
